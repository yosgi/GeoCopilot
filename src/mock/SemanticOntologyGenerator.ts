      ...
      switch (subcategory) {
        ...
        case '825': // Electrical
          if (concept.includes('motor')) {
            ...
            specs.voltage = (classData as { voltage: string }).voltage;
            specs.applications = (classData as { applications: string[] }).applications;
        ...
        case '619': // Heat Exchangers
          if (concept.includes('hx')) {
            ...
            const tubeMatIndex = Math.floor(this.seededRandom(seed + 2, 0, 3));
            const tubeMaterials = ['carbon_steel', 'stainless_steel_316l', 'titanium'];
            specs.tube_material = tubeMaterials[tubeMatIndex];
            specs.design_type = ['U-tube', 'straight_tube', 'floating_head'][Math.floor(this.seededRandom(seed + 3, 0, 3))];
          }
          break;
      }
